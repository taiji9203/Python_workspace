...
def randomPhoto():
    ...
    for i in range(0, XSIZE):
        for k in range(0, YSIZE):
            data = inImage[i][k]
            newData = data + addValue
            if  data > 255 :
                # newData = 255
                newData = inImage2[1][k]
            elif data < 0 :
                newData = 0
            else :
                newData = data
    ...
    # brightness = random.randint(-255, 255)

    # for i in range(0, XSIZE):
    #     for k in range(0, YSIZE):
    #         data = inImage[i][k]
    #         newData = data + brightness
    #         if newData < 0:
    #             newData = 0
    #         elif newData > 255:
    #             newData = 255
...
if __name__ == "__main__" :
    ...
    photoMenu.add_command(label="랜덤 이미지", command=randomPhoto)
    window.mainloop()
